...
def generate_ring_from_image(brigthfield_path,dapi_path,
...
#      coordinates of the nuclei centers clockwise
    """
    imgData = dataImage(brigthfield_path, dapi_path, threshold, blur)
    inners, outers = imgData.extract_membranes()
    centers = imgData.extract_nuclei()
    
    #rolling mean
    inners = pd.rolling_mean(inners, rol_window, min_periods=1)
    outers = pd.rolling_mean(outers, rol_window, min_periods=1)
    ...
    org_center = imgData.center_outside-np.full(2,imgData.shape[0]/2.0)
...
class dataImage:
    ...
    def __init__(self, brigthfield_path, CP_dapi_path, threshold, blur):
        """
        Parameters
        ----------
        brigthfield_path : string
          path to the brigthfield image
        CP_dapi_path : string
          path to the csv output file from CellProfiler
        threshold : int >=0
          value of the threshold to apply to the brigthfield image
        blur : int >=0
          value of the gaussian blur to apply to the brigthfield image
    
        """

        self.brigthfield = cv.imread(brigthfield_path,cv.IMREAD_GRAYSCALE)
        self.dapi_df = pd.read_csv(CP_dapi_path)
        self.shape = self.brigthfield.shape
    ...
    def extract_membranes(self):
        ...
        #28
        ret,img = cv.threshold(img,self.threshold,255,0)
        #9
        ...
        retained_contour = contours[membrane_ind]
        
        circles = np.array((cv.minEnclosingCircle(retained_contour[0]) ,
                            cv.minEnclosingCircle(retained_contour[1])))
        
        centers = circles[:,0]
        radii = circles[:,1]
        
        retained_contour = np.array((np.squeeze(retained_contour[0]),
                                     np.squeeze(retained_contour[1])))
        
        dist = retained_contour - \
                np.array((np.full(retained_contour[0].shape,centers[0]),
                          np.full(retained_contour[1].shape,centers[1])))
                
        norm = np.array((np.mean(np.linalg.norm(dist[0],axis=1)),
                np.mean(np.linalg.norm(dist[1],axis=1))))
                
        inside, outside = retained_contour[np.argsort(norm)]
        
        self.center_outside, self.center_inside = centers[np.argsort(norm)]
        self.rOut, self.rIn = radii[np.argsort(norm)]
        
        inside = np.concatenate((inside,_fill_gaps(inside,2)))
        outside =  np.concatenate((outside,_fill_gaps(outside,2)))        
        
        self.raw_inside = inside
        
        inside = (inside - np.ones(inside.shape)*(img.shape[0]/2.0,
                  img.shape[1]/2.0))
        outside = (outside - np.ones(outside.shape)*(img.shape[0]/2.0,
                   img.shape[1]/2.0))
    ...
    def extract_nuclei(self): 
        ...
        Return
        ----------
        clockwiseCenters : np.array of shape (Nf, 2)
          coordinates of the nuclei centers ordonned clockwise.
    
        """ 
        try:
          org_center = self.center_inside
          raw_inside = self.raw_inside
        except NameError:
          print("It seems like the center of the organoid and/or its inner \
                contour is undefined. \nsegment2D.dataImage.extract_membranes\
                should be run before running segment2D.dataImage.\
                extract_nuclei.\nRunning segment2D.dataImage.extract_membranes")
          self.extract_membranes(self)
        centers =  np.column_stack((self.dapi_df['AreaShape_Center_X'], 
                                    self.dapi_df['AreaShape_Center_Y']))
        #delete the centers which are not inside the organo (possibly newborn cells ??)
        c2m = np.array([np.min(np.linalg.norm(
                                np.full(raw_inside.shape,center)-
                                raw_inside,axis=1)) for center in centers])
#        for center in centers:
#            c2m = np.min(np.linalg.norm(np.full(raw_inside.shape,center)-raw_inside,axis=1),axis=1)
        to_del= np.argwhere(c2m>2*np.mean(c2m))
        centers = np.delete(centers, to_del,0)
        centers = [(i[0],i[1]) for i in centers]
        #for each cell defined by its center, compute its closest clockwise neighbor   
        neighbors = _findNeighbor(centers, org_center)
        ...
        clockwiseCenters -= np.full(np.asarray(clockwiseCenters).shape,
        ...
        x=[]
        [x.append(tuple(r)) for r in clockwiseCenters if tuple(r) not in x]
        self.clockwiseCenters = np.array(x)
        return self.clockwiseCenters
...
def _findNeighbor(centers, org_center):
    '''
    Parameters
    ----------
    ...
    for i in centers:
        ...
        for j in centers:
            ...
            if not ii[0]==jj[0] and not ii[1]==jj[1]:
                if ii[0]*jj[1]-ii[1]*jj[0] > 0 \
                    and np.sqrt((ii[0]-jj[0])**2 + (ii[1]-jj[1])**2)<min1:
