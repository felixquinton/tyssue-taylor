...
def generate_ring_from_image(
    ...
    clockwise_centers = _star_convex_polynoms(dapi_path, membrane_dic, scp_model_path)
    ...
    inner_vs, outer_vs = get_bissecting_vertices(
        clockwise_centers, inners, outers, org_center
    )

    organo = generate_ring(nb_cells, membrane_dic["rIn"], membrane_dic["rOut"])

    organo.vert_df.loc[organo.apical_verts, organo.coords] = (
        inner_vs[::-1] - np.full(inner_vs.shape, org_center)
    ) * 0.323
    organo.vert_df.loc[organo.basal_verts, organo.coords] = (
        outer_vs[::-1] - np.full(outer_vs.shape, org_center)
    ) * 0.323
...
def _star_convex_polynoms(dapi_path, membrane_dic, model_path):
    ...
    model_sc = StarDist2D(None, name="stardist_shape_completion", basedir=model_path)
    ...
    clockwise_centers = _quick_clockwise(cleaned, phi, rho, membrane_dic["rIn"])
...
def extract_membranes(brightfield_path, threshold=2, blur=9):
    ...
    circles = np.array(
        (
            cv.minEnclosingCircle(retained_contours[0]),
    ...
    centers = circles[:, 0]
    radii = circles[:, 1]
    inside, outside, res_dic = _recognize_in_from_out(retained_contours, centers, radii)
    ...
    res_dic["inside"] = inside - np.ones(inside.shape) * (
        img.shape[0] / 2.0,
        img.shape[1] / 2.0,
    )
    res_dic["outside"] = outside - np.ones(outside.shape) * (
        img.shape[0] / 2.0,
        img.shape[1] / 2.0,
    )
...
def get_bissecting_vertices(centers, inners, outers, org_center):
    ...
    """
    theta_centers = np.arctan2(
        centers[:, 1] - org_center[1], centers[:, 0] - org_center[0]
    )
    ...
    bissect[dtheta < -np.pi] += np.pi
    theta_inners = np.arctan2(
        inners[:, 1] - org_center[1], inners[:, 0] - org_center[0]
    )
    theta_outers = np.arctan2(
        outers[:, 1] - org_center[1], outers[:, 0] - org_center[0]
    )
...
def normalize_scale(organo, geom, refer="area"):
    ...
    organo.settings["lumen_prefered_vol"] *= (
        organo.settings["lumen_volume"] / old_lumen_vol
    )
    return organo
...
def _recognize_in_from_out(retained_contours, centers, radii):
    ...
    """
    retained_contours = np.array(
        (retained_contours[0].squeeze(), retained_contours[1].squeeze())
    )
...
def _fill_gaps(contour, gap_dist):
    ...
    for gap in gaps:
        ...
        res = np.concatenate((res, np.column_stack((x_new, y_new))))
    return res
