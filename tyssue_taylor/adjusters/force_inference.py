...
def infer_forces(organo, sup_param="areas", t_sum=0.01, verbose=False):
    ...
    *****************
    ...
    if sup_param == "areas":
        dic_res = {
            "tensions": system_sol[: int(organo.Ne * 0.75)],
...
def opt_sum_lambda(eptm, method="golden"):
    ...
    *****************
    """
    tmp_eptm = eptm.copy()
    cst_opt_result = minimize_scalar(
        _opt_cst_obj, args=(tmp_eptm.copy()), method=method
    )
...
def _opt_cst_obj(sum_tensions, eptm):
    ...
    *****************
    """
    th_eptm = eptm.copy()
    exp_eptm = _tmp_infer_forces(th_eptm, sum_tensions).copy()
...
def _coef_matrix(organo, sup_param="areas", t_sum=0.01):
    ...
    *****************
    ...
    srce_rows = np.concatenate([organo.edge_df.srce, organo.edge_df.srce + organo.Nv])

    trgt_rows = np.concatenate([organo.edge_df.trgt, organo.edge_df.trgt + organo.Nv])
    ...
    coef = np.r_[
        ...
        _t_per_cell_coefs(
            organo,
            _get_sim_param(
                organo.Nf, organo.settings["R_in"], organo.settings["R_out"], t_sum
            ),
            _infer_pol(organo),
            sup_param=sup_param,
        ),
...
def _areas_coefs(organo):
    ...
    grad_l_srce, grad_l_trgt = lumen_area_grad(organo)

    kl = organo.settings["lumen_elasticity"]

    Nv, Ne, Nf = organo.Nv, organo.Ne, organo.Nf

    mat_shape = (2 * Nv, Nf + 1)

    mat = np.zeros(mat_shape)
    lumen_index = Nf * np.ones(Ne)
    srce, trgt, face = organo.edge_df[["srce", "trgt", "face"]].values.T

    for i, c in enumerate(organo.coords):

        mat += sparse.coo_matrix(
            (grad_srce["g" + c].values, (srce + i * Nv, face)), shape=mat_shape
        ).todense()
        mat += sparse.coo_matrix(
            (grad_trgt["g" + c].values, (trgt + i * Nv, face)), shape=mat_shape
        ).todense()

        mat += sparse.coo_matrix(
            (kl * grad_l_srce["g" + c].values, (srce + i * Nv, lumen_index)),
            shape=mat_shape,
        ).todense()

        mat += sparse.coo_matrix(
            (kl * grad_l_trgt["g" + c].values, (trgt + i * Nv, lumen_index)),
            shape=mat_shape,
        ).todense()

    return mat

def _t_per_cell_coefs(eptm, params_in_sym_mesh, polar_coefs, sup_param="areas"):
    ...
    *****************
    ...
    col_inds = np.c_[
        ...
        (base_col_ind + 1) % (eptm.Nf) + 2 * eptm.Nf,
    ].flatten()
    ...
    coef_matrix = sparse.coo_matrix(
        (coefs_tables, (row_inds, col_inds)), shape=matrix_shape
    )
...
def _right_side(eptm, t_sum=0.01):
    ...
    *****************
    """
    params_in_sym_mesh = _get_sim_param(
        eptm.Nf, eptm.settings["R_in"], eptm.settings["R_out"], sum_lbda=t_sum
    )

    # Computing the Farhadifar constant (multiplied by 4 to
    # account for the 4 edges of a cell)
    far_cste = 4 * (
        (params_in_sym_mesh[1:].sum() + params_in_sym_mesh[-1])
        / 3
        / (abs(eptm.face_df.area.mean() + params_in_sym_mesh[0])) ** 1.5
    )
...
def _infer_pol(eptm):
    ...
    *****************
    ...
    """
    non_lateral_edges = np.concatenate((eptm.apical_edges, eptm.basal_edges))
    ...
    # cell center
    dist = np.divide(
        np.abs(np.multiply(a, cell_centers[:, 0]) - cell_centers[:, 1] + b),
        np.sqrt(a ** 2 + 1),
    )

    # Computing the same distance in the symetric mesh
    sym_height = (eptm.settings["R_out"] - eptm.settings["R_in"]) * np.cos(
        np.pi / eptm.Nf
    )
...
def _nnls_model(organo, sup_param, t_sum, verbose):
    ...
    *****************
    """
    res, _ = nnls(_coef_matrix(organo, sup_param, t_sum), _right_side(organo, t_sum))
    if verbose:
        print(res)
    return res


def _print_solving_results(organo, fi_res, coefs, constant, sup_param=None):
    """Used to print the results when the module is run from the console.
    """
    print("Ideal scale factor: ", np.sum(ORGANO.edge_df.line_tension[: 3 * ORGANO.Nf]))
    print("A :\n", coefs)
    print("b :", constant)
    ...
    if sup_param == "areas":
        vect_param = np.concatenate(
            (
                vect_param,
                organo.face_df.area - organo.face_df.prefered_area,
                [
    ...
    if COEFS.shape[1] == vect_param.shape[0]:
        ...
        print("||At-b||: ", np.linalg.norm(np.dot(coefs, vect_param) - constant))
...
if __name__ == "__main__":
    ...
    RES_INFERENCE = infer_forces(
        ORGANO, method=METHOD, sup_param=SUP_PARAM, no_scale=False
    )
    print(ORGANO.vert_df.loc[:, ("x", "y")])
